...
/**
 * feedback.php — the mentee's Feedback page.
 *
 * The mirror of the mentor's page: this one reads `mentee_reviews`, the
 * reviews mentors leave about their mentees, and uses that direction's four
 * aspects. Layout, cards and helpers are shared (includes/feedback_page.php)
 * so the two roles cannot drift apart.
 *
 * Giving feedback still happens on the review form — reachable here through
 * the "Review your mentors" control, the same way the mentor page links to
 * its review form.
 */
...
/*
 * The four aspects a mentor rates a mentee on. Labels match the ones the
 * review form shows under "Mentor assessing you", so a mentee reads the same
 * wording in both places. Order drives the breakdown bars, the category
 * filter, and the tie-break when a review scores two aspects equally.
 */
$CATS = [
    'preparedness'  => ['label' => 'Preparedness',               'color' => 'var(--info)',    'bg' => 'var(--info-bg)'],
    'participation' => ['label' => 'Participation & Engagement', 'color' => 'var(--success)', 'bg' => 'var(--success-bg)'],
    'communication' => ['label' => 'Communication',              'color' => 'var(--warning)', 'bg' => 'var(--warning-bg)'],
    'receptiveness' => ['label' => 'Openness to Guidance',       'color' => 'var(--purple)',  'bg' => 'var(--purple-bg)'],
];

/*
 * The reviews. `mentee_reviews.session_id` points at a session_request;
 * duration and session type live on the availability slot that request was
 * booked against, matched the way the rest of the app matches them (mentor +
...
foreach ($rows as $r) {
    ...
    $mentorIds[(int)$r['mentor_id']] = true;
...
if ($total > 0 && $topKey !== null) {
    $insights[] = ['icon' => 'chart', 'text' => 'Mentors rate your ' . $CATS[$topKey]['label'] . ' highest, at ' . number_format($catAvg[$topKey], 1) . '.'];
}
    ...
    <div class="app">
        ...
        <main class="main">
            ...
            <div class="page-hd fbk-hd">
                <div>
                    <h1>My Feedback</h1>
                    <p>Ratings and comments from your mentors</p>
                </div>
                <a href="<?= htmlspecialchars($reviewUrl) ?>" class="fbk-review-cta">
            ...
            <div class="fbk-stats">
                ...
                <div class="card fbk-stat">
                    ...
                    <div class="fbk-stat-body">
                        <div class="fbk-stat-v"><?= $uniqueMentors ?></div>
                        <div class="fbk-stat-k">Unique Mentors</div>
                        <?php if ($workedCount > 0): ?>
                            <span class="fbk-stat-sub">of <?= $workedCount ?> you've worked with</span>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="card fbk-stat">
                    ...
                    <div class="fbk-stat-body">
                        <?php if ($topKey !== null): ?>
                            ...
                            <div class="fbk-stat-k">Top Strength</div>
                ...
                <div class="fbk-side">
                    <div class="card card-p">
                        ...
                        <?php if ($rated): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p style="margin:0;font-size:13px;color:var(--gray-400);">No aspect scores yet — they appear once a mentor rates a session.</p>
                        <?php endif; ?>
                    </div>
                    ...
                    <?php if ($insights): ?>
                        <div class="card card-p">
                            ...
                            <?php endforeach; ?>
                            <p class="fbk-quote">&ldquo;Every session is a step forward.&rdquo;</p>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div>
                    <div class="fbk-list-hd">
                        ...
                        <?php if ($total > 0): ?>
                            <div class="fbk-filters">
                                ...
                                <label class="fbk-filter">
                                    <?= fbk_svg('tag') ?>
                                    <span class="sr-only">Filter by aspect</span>
                                    <select id="fbkCat">
                    ...
                    <div class="fbk-reviews" id="fbkList">
                        <?php foreach ($rows as $r):
                            ...
                            // The chip names the aspect this mentor scored
                                ...
                                <div class="fbk-rev-hd">
                                    ...
                                    <div class="fbk-more">
                                        ...
                                        <div class="fbk-menu" role="menu">
                                            <a role="menuitem" href="<?= htmlspecialchars($messagesUrl . '?chat=' . (int)$r['mentor_id']) ?>">
                    ...
                    <?php if ($total === 0): ?>
                        <div class="card fbk-empty">
                            ...
                            <p style="margin-top:4px;font-size:12.5px;">Reviews appear here once a mentor rates a session you attended.</p>
                        </div>
                    <?php else: ?>
